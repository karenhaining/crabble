...
DIR = "vision/original_data/holder_data"
...
for filename in os.listdir(DIR):
    ...
    try:
        ...
        hp.set_image(filepath)
        cropped_hand = hp.crop_to_hand()

    except:
        print(f"FAILED IMAGE: {filename}")
        continue

    split = cv2.split(cv2.cvtColor(cropped_hand, cv2.COLOR_RGB2HSV))
    ...
    for i in range(config.HAND_SIZE):
        ...
        w = 45
        pic = cv2.getRectSubPix(threshold.copy(), [w, w], (x, y))
